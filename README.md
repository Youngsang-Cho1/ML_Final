...
- Spectral & K-Means Clustering: We map each song into a vector space using its 12 numerical audio features and compute Laplacian Coordinates using an RBF Kernel. K-Means clustering is then run on these Laplacian mappings to group songs with similar audio characteristics into different categories. 
...
- Demo: A deployed web application built with Streamlit, now featuring live audio previews for any queried or recommended song using YouTube fetching!
